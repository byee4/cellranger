...
RUN apt-get install -y nano unzip zip

COPY bin               /opt/
COPY demo              /opt/
COPY cellranger-2.0.0  /opt/
COPY LICENSE           /opt/

ENV PATH /opt/cellranger-2.0.0:/opt/bin:/opt/conda/bin:/usr/local/bin:/usr/bin:/bin
ENV REFDATA ./

CMD echo "cellranger-2.0.0"
